...
public class CarControllerJerseyGetInIntegrationTest {
	...
	public void testGetIn() {
		...
		ResponseEntity<HappyParkingResponse> entity = restTemplate.postForEntity(url, getRequest(requestJson),
	...
	public void testGetInThereAreNotSpaceToParking() {
		...
		assertThat(entity.getBody()).isInstanceOf(HappyParkingResponse.class);
		assertThat(entity.getBody().getStatus()).isEqualTo(Status.NOT_FOUND.getStatusCode());
		assertThat(entity.getBody().getContent()).isEqualTo("There are not space for cars");
	}
	...
	public void testGetInNotAbleDay() {
		...
		assertThat(entity.getBody()).isInstanceOf(HappyParkingResponse.class);
		assertThat(entity.getBody().getStatus()).isEqualTo(Status.NOT_FOUND.getStatusCode());
		assertThat(entity.getBody().getContent()).isEqualTo("The car can not get into parking lot because is not a able day");
	}
	...
	private void setupDatabase(String plate) {
		...
		Car car = CarTestBuilder.create().withPlate(plate).withIsParking().build();
